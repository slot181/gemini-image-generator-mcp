...
async def save_or_upload_image(image_data: bytes, filename_base: str) -> str:
    ...
    if CF_IMGBED_UPLOAD_URL and CF_IMGBED_API_KEY:
        ...
        try:
            ...
            # Return combined info if both succeeded
            if local_save_path:
                 return f"Image uploaded to: {uploaded_url}\nAlso saved locally at: {local_save_path}"
            else:
                 # Should only happen if local save failed after successful upload
    ...
    if OUTPUT_IMAGE_PATH:
         ...
         # Use the unique filename for local save
         return await _save_locally(image_data, filename_with_unique_ext)
    else:
