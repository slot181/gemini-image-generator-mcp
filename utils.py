...
async def _save_locally(image_data: bytes, filename_with_unique_ext: str) -> str:
    """Helper function to save image data locally and return the filename."""
    if not OUTPUT_IMAGE_PATH:
         raise ValueError("Local save requested but OUTPUT_IMAGE_PATH is not configured.")
    try:
        # Open image from bytes
        image = PIL.Image.open(io.BytesIO(image_data))
        # Save the image locally using the unique filename
        local_image_path = os.path.join(OUTPUT_IMAGE_PATH, filename_with_unique_ext)
        image.save(local_image_path)
        logger.info(f"Image saved locally to {local_image_path}")
        # Return only the filename
        return filename_with_unique_ext
    except Exception as e:
        logger.error(f"Error saving image locally: {str(e)}")
        raise
async def save_or_upload_image(image_data: bytes, filename_base: str) -> str:
    """Save image locally or upload to CloudFlare-ImgBed, returning URL or filename.

    Attempts upload first if configured.
    - If upload succeeds, returns the public URL.
    - If upload fails or is not configured, attempts local save (if configured).
    - If local save succeeds, returns the unique filename used for saving.

    Args:
        image_data: Raw image data (bytes).
        filename_base: Base string for the filename (without extension), generated by AI.

    Returns:
        str: The public URL of the uploaded image (if upload succeeded),
             OR the unique filename of the locally saved image (if upload failed/disabled but local save succeeded).

    Raises:
        ValueError: If neither ImgBed nor local saving is configured or available.
        Exception: If both upload (if attempted) and local save fail.
    """
    ...
    if CF_IMGBED_UPLOAD_URL and CF_IMGBED_API_KEY:
        ...
        try:
            # Use the unique filename for upload
            uploaded_url = await upload_to_cf_imgbed(image_data, filename_with_unique_ext)
            ...
            # Return the URL as the primary result of successful upload
            return uploaded_url
    ...
    if OUTPUT_IMAGE_PATH:
         ...
         # This block is reached if ImgBed is not configured OR if upload failed and fallback is enabled
         # _save_locally returns the filename directly
         saved_filename = await _save_locally(image_data, filename_with_unique_ext)
         # Return the filename as the result of successful local save
