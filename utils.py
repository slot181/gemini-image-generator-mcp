...
async def save_or_upload_image(image_data: bytes, filename_base: str) -> str:
    """Save image locally or upload to CloudFlare-ImgBed, returning the generated filename.

    Attempts upload first if configured. If upload succeeds or fails but local save
    is configured and succeeds (either as primary or fallback), the unique filename
    used for saving/uploading is returned.
    ...
    Returns:
        str: The unique filename (e.g., "ai_image_20240101_120000_abcdef12.png")
             used for saving the image locally or attempting the upload.
    ...
    if CF_IMGBED_UPLOAD_URL and CF_IMGBED_API_KEY:
        ...
        try:
            ...
            # Regardless of whether local save succeeded after upload, return the filename used.
            # The primary success indicator here is the upload.
            return filename_with_unique_ext
    ...
    if OUTPUT_IMAGE_PATH:
         ...
         # Use the unique filename for local save
         # _save_locally now returns the filename directly
